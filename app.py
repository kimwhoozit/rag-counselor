...
# Multi-tab layout
tab1, tab2, tab3 = st.tabs(["💬 대화 및 서류 검토 (Chat)", "📚 지식 관리 & 동기화", "⚙️ 시스템 가이드 & 관리"])
...
with tab2:
    ...
    with col_right:
        ...
        st.markdown(f"""
        - **총 누적 지식 수**: `{total_docs}` 개
          - **파일 업로드 지식**: `{doc_count}` 개
          - **AI 학습 피드백 지식 (피드백 데이터)**: `{qa_count}` 개
        """)
        
        st.info("💡 **안내**: 로컬 파일의 삭제는 `documents` 폴더에서 파일을 직접 삭제한 후 왼쪽의 '동기화 실행'을 눌러 반영하는 것을 권장합니다.")
        ...
        if all_docs:
            ...
            if selected_to_inspect:
                ...
                if st.button("🗑️ 선택한 지식 삭제", type="secondary", use_container_width=True):
                    database.delete_document(selected_to_inspect['id'])
                    # If it's a file, remove its indexed record so it will be recognized as "newly added" if synced again
...
with tab3:
    ...
    with col_guide:
        st.subheader("📖 프로그램 작동 구조 안내")
        st.write("""
        본 프로그램은 **성장형 RAG(Retrieval-Augmented Generation)** 아키텍처에 기반하여 설계되었습니다.
        
        1. **다중 소스 동기화 (로컬 폴더 및 구글 드라이브)**
           - `documents` 폴더 내에 직접 파일을 배치하거나, 공유 폴더 형태로 구글 드라이브와 실시간 동기화하여 시방서, 리스크 보고서, 계약서 등을 데이터베이스에 자동 임베딩 인덱싱합니다.
           - 한글 문서(`.hwpx`), 엑셀(`.xlsx`, `.xls`), 워드(`.docx`), PDF(`.pdf`), 텍스트(`.txt`, `.md`) 파일 포맷을 모두 파싱합니다.
           
        2. **실시간 외부 기준 검토 (Google Search Grounding)**
           - 질문 내용 중에 법령 기준이나 규정이 포함되어 있으면 AI가 구글 실시간 웹 검색을 사용하여 답변에 참고한 원본 링크들을 보여줍니다.
    ...
    with col_admin:
        st.subheader("👥 사용자 계정 권한 관리 (Admin Panel)")
        
        # Display account administration options if user is admin
        if st.session_state.role == "admin":
            ...
            if user_to_delete:
                if st.button(f"🗑️ `{user_to_delete}` 계정 영구 삭제"):
    ...
    if st.button("🔌 API 연결 및 모델 리스트 검사", use_container_width=True):
        api_key = st.session_state.get("gemini_api_key")
        if not api_key:
            ...
            with st.spinner("Gemini API에서 사용 가능한 모델 목록을 조회 중..."):
                try:
                    ...
                    # Split into generation and embedding models
                    embed_models = []
                    gen_models = []
                    for m in models:
